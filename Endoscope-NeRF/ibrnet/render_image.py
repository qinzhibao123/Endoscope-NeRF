...
def render_single_image(ray_sampler, ray_batch, model, projector, chunk_size, N_samples, inv_uniform=False, N_importance=0, det=False, white_bkgd=False, render_stride=1, featmaps=None):
    ...
    N_rays = ray_batch['ray_o'].shape[0]  #756*1008

    for i in range(0, N_rays, chunk_size):  #chunk_size=1024  N_rays=190512   每批次加载1024条射线
        chunk = OrderedDict()
        for k in ray_batch:
            if k in ['camera', 'depth_range', 'src_rgbs', 'src_cameras']:
                ...
                chunk[k] = None
        #开始进行射线渲染********************************
        ret = render_rays(chunk, model, featmaps, projector=projector, N_samples=N_samples, inv_uniform=inv_uniform, N_importance=N_importance, det=det, white_bkgd=white_bkgd)

        # handle both coarse and fine outputs
        # cache chunk results on cpu
        if i == 0:  #如果为0，即刚开始渲染，先将all_ret清空
